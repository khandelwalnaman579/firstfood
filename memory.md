...
### Still open - decide before Phase 2

- **Next.js `NEXT_PUBLIC_*` build-time API URL** — real risk once Phase 2
  wires up frontend↔backend calls: `NEXT_PUBLIC_API_BASE_URL` gets baked
  into the browser bundle at `npm run build` time, so changing it at
  container runtime (as `infra/docker-compose.yml` currently does via an
  ...
  reverse proxy) now live in README "Frontend API URL: build-time vs.
  runtime" - decide there the moment Phase 2 adds a real client-side API
  call, and update both README and this section with the outcome.
